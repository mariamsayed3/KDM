...
from utils.utils import create_exp_dir, \
    get_experiment_dataloaders, init_obj, compute_confusion_matrix, test_running_time, compute_eval_from_cm
...
def train_epoch(optimizer, student_model, teacher_model, model_id, train_loader, loss_fn, metric, device):
    """
    ...
    for step, minibatch in enumerate(pbar):
        ...
        with torch.no_grad():
            #teacher_model.to('cuda:1')
            teacher_f, teacher_o = teacher_model(x,early_exit=model_id)
...
def val_epoch(student_model, teacher_model, model_id, val_loader, loss_fn, metric, device):
    """
...
def training(cfg, optimizer, student_model, teacher_model, train_loader, val_loader, loss_fn,
    ...
    for epoch in range(last_epoch, n_epochs):
        ...
        print(f"\nDistilling the {model_id + 1}-th teacher to the student")
        print("-----------------------------------")
        # Train a epoch
        with comet.train():
            train_loss, train_performance = train_epoch(optimizer, student_model, teacher_model, model_id,
        ...
        # Validate a epoch
        with comet.validate():
            val_loss, val_performance = val_epoch(student_model, teacher_model, model_id, val_loader, loss_fn,
                                                  metric, device)
...
def testing(model, classes, test_loader, metric, device, comet, save_dir):
    ...
    print(f"Testing all-mIoU clf: {mean_IoU}")
    print(f"Testing all-acc clf: {pixel_acc}")
    print(f"Testing all-OA clf: {mean_acc}")
    print(f"IoU Array", IoU_array)
    comet.log_metric(f"test_iou_array", IoU_array)
    ...
    print(f"Average Accuracy: {pixel_acc:.4f}")
    comet.log_metric(f"test_aa", avg_aa)

    avg_kappa = kappa / count_kappa
    print(f"Average Kappa Clf: {kappa:.4f}")
